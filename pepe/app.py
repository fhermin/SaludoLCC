...
def capture_images_t(name):
    ...
    if not cap.isOpened():
        st.error("La cámara no está abierta.")
        return
    ...
    while images_taken < 5:
        ret, frame = cap.read()
        if not ret:
            st.error("Error al capturar la imagen de la cámara.")
            break
...
def main():
    ...
    st.title("Aplicación de Detección Facial")
    st.write("Esta es una aplicación de ejemplo para la detección facial y captura de imágenes usando Streamlit.")
